...
from app.visual_similarity import (
    ...
    compute_image_dhash, compute_dhash_similarity
)
...
class ScanPipeline:
    ...
    async def _stage_2_feature_extraction(self, ctx: PipelineContext):
        ...
        # 3. Visual embedding & ResNet / dHash matching
        def run_vis_match():
            if ctx.screenshot_bytes and self.visual_store is not None:
                return self.visual_store.find_best_match(ctx.screenshot_bytes)
            return 0.0, None
    ...
    async def _stage_3_fusion_and_attribution(self, ctx: PipelineContext):
        ...
        # P3. High-confidence Deep visual feature match (if confident >= 0.70 AND supported by DOM)
        # P4. Stand-Alone Visual Feature match (if >= 0.85)
        # Default: None
        ...
        elif ctx.vis_matched_brand and ctx.vis_score >= 0.70 and ctx.dom_score >= 0.30:
            matched_brand = ctx.vis_matched_brand
        elif ctx.vis_matched_brand and ctx.vis_score >= 0.85:
            matched_brand = ctx.vis_matched_brand
        else:
            matched_brand = None
        ...
        if matched_brand is not None:
            s_dom = ctx.dom_score if ctx.dom_matched_brand == matched_brand else 0.0
            s_vis = ctx.vis_score if ctx.vis_matched_brand == matched_brand else 0.0
        else:
            s_dom = 0.0
            s_vis = 0.0
